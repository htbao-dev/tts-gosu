...
class FriendRepository {
  ...
  Future<FriendRequestStatus> acceptFriendRequest(String friendID) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? accessToken = prefs.getString('accessToken');
    final response = await http
        .post(Uri.parse('$apiUrl/user/accept-friend-request'), headers: {
      // 'Content-Type': 'application/json',
      'Authorization': 'Bearer $accessToken',
    }, body: {
      'friendId': friendID,
  ...
  Future<FriendRequestStatus> unfriend(String friendId) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? accessToken = prefs.getString('accessToken');
    final response =
        await http.post(Uri.parse('$apiUrl/user/unfriend'), headers: {
      // 'Content-Type': 'application/json',
      'Authorization': 'Bearer $accessToken',
    }, body: {
      'friendId': friendId,
